...
from deerflow.config.context import get_app_config, init_app_config
...
def test_init_replaces_previous(tmp_path, monkeypatch):
    ...
    init_app_config(config_a)
    assert get_app_config().models[0].name == "model-a"

    _write_config(config_path, model_name="model-b", supports_thinking=True)
    config_b = AppConfig.from_file(str(config_path))
    init_app_config(config_b)
    assert get_app_config().models[0].name == "model-b"
    assert get_app_config() is config_b
