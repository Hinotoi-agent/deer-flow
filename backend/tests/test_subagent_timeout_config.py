...
- AppConfig.subagents field access via get_app_config()
...
# ---------------------------------------------------------------------------
# AppConfig.subagents via get_app_config()
# ---------------------------------------------------------------------------
...
class TestAppConfigSubagents:
    def test_load_global_timeout(self):
        ...
        with patch("deerflow.config.context.get_app_config", return_value=cfg):
            from deerflow.config.context import get_app_config

            sub = get_app_config().subagents
