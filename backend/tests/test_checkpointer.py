...
class TestGetCheckpointer:
    def test_returns_in_memory_saver_when_not_configured(self):
        ...
        with patch("deerflow.agents.checkpointer.provider.get_app_config", return_value=_make_config()):
            cp = get_checkpointer()
    ...
    def test_memory_returns_in_memory_saver(self):
        ...
        with patch("deerflow.agents.checkpointer.provider.get_app_config", return_value=cfg):
            cp = get_checkpointer()
        assert isinstance(cp, InMemorySaver)
    ...
    def test_sqlite_raises_when_package_missing(self):
        ...
        with (
            patch("deerflow.agents.checkpointer.provider.get_app_config", return_value=cfg),
